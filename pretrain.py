...
class TextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
        ...
        # import pdb;pdb.set_trace()   #### block_size = 起初设置好的max_seq_length-2
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def collate_fn(batch):
        texts = batch  # 假设 batch 是 (text, label) 对的列表
        # import pdb;pdb.set_trace()
        padded_texts = pad_sequence(texts, batch_first=True, padding_value=tokenizer.pad_token_id)
        # labels = torch.tensor(labels)
        return padded_texts

    # train_dataset_pad = pad_sequence(train_dataset, batch_first=True, padding_value=tokenizer.pad_token_id)
    train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset_pad)
    ...
    for e in train_iterator:
        ...
        if best_eval_loss > eval_result['eval_loss']:
            ...
            # torch.save(best_model.state_dict(), os.path.join(output_dir, pretrain_model_name+'_'+str(args.block_size)+".pth"))
        else:
            ###  如果超过20轮没有提升，则early_stopping早停
            optim_counter += 1
            if optim_counter > args.early_stop:
                print('EarlyStopping!!! at epoch: {}'.format(e))
                break
...
# 'Usage: python pretrain.py --do_eval --do_train'
def main():
    global pretrain_model_name
    max_seq_length = 100  ###对应参数p,字节数量
    TRAIN_FILE = f"./DataCache/1t-30n-IoT_pretrain_train.txt"
    EVAL_FILE = f"./DataCache/1t-30n-IoT_pretrain_test.txt"
    pretrain_model_name ="1t-30n-IoT_100"
    ...
    parser.add_argument(
        "--early_stop", default=early_stop, type=int,
    )
    # Other parameters
    parser.add_argument(
        "--eval_data_file",
