...
class ModelConfig():
    def __init__(self, config):
        ...
        model_mlm.load_state_dict(torch.load(config.pretrainModel_path,map_location=config.device))
...
class PEAN(nn.Module):
    def __init__(self, config):
        ...
        self.length_emb_dim = config.length_emb_dim
        ...
        if config.feature == "raw":
            ...
            self.length_embedding = nn.Embedding(self.length_emb_dim, config.length_emb_size, padding_idx=0)
        ...
        elif config.feature == "raw+length":
            self.length_embedding = nn.Embedding(self.length_emb_dim, config.length_emb_size, padding_idx=0)
            self.lenlstm = nn.LSTM(config.length_emb_size, config.lenlstmhidden_size, config.num_layers,
                                   bidirectional=True, batch_first=True, dropout=config.dropout)
            ...
            if config.method == "lstm":
                ...
                self.fc = nn.Linear((self.emb_size * config.pad_num + config.lenlstmhidden_size * 2),
                                    config.num_classes)
                self.fc01 = nn.Linear(self.emb_size * config.pad_num, config.num_classes)
    ...
    def forward(self, x):
        ...
        if ("raw" in config.feature) or config.feature == "ensemble":
            ...
            if config.embway == "pretrain":
                for i in range(config.pad_num):
                    pooled = self.emb((traffic_bytes_idss[:, i, :]))[1]
        ...
        if ("length" in config.feature) or config.feature == "ensemble":
            ...
            out2 = torch.cat((final_hidden_state[-2, :, :], final_hidden_state[-1, :, :]), dim=1)
